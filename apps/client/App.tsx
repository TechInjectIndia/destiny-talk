...
import { auth, db, isConfigured } from '../../packages/utils/firebase';
import { ai } from '../../packages/utils/ai';
import { logEvent } from '../../packages/utils/analytics';
import { generateNumerologyProfile, NumerologyReport } from '../../packages/core/src/numerology';
import { Button, Card, Input, PaymentModal } from '../../packages/ui/index';
...
const WalletView = ({ userId }: { userId: string }) => {
  ...
  const initiatePayment = async (amount: number) => {
    try {
        ...
        return orderRef.id;
    } catch (e: any) { alert("Failed to start payment: " + e.message); }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (isAnswer) {
        ...
        await runTransaction(db, async (t) => {
           const walletDoc = await t.get(walletRef);
           if (!walletDoc.exists || walletDoc.data().balance < 10) throw new Error("Insufficient Balance for Answer");
           
           t.update(walletRef, { balance: walletDoc.data().balance - 10, updatedAt: serverTimestamp() });
...
const ReportGenerator = ({ profile, coreNumbers, existingReport, walletBalance }: { profile: UserProfile, coreNumbers: NumerologyReport, existingReport: FullReport | null, walletBalance: number }) => {
  ...
  const handleUnlockReport = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (t) => {
        const walletDoc = await t.get(walletRef);
        if (!walletDoc.exists || walletDoc.data().balance < 100) throw new Error("Insufficient funds");
        t.update(walletRef, { balance: walletDoc.data().balance - 100, updatedAt: serverTimestamp() });
